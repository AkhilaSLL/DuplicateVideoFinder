...
- **The tree's ☐/☑ marks are text, not real widgets, so they need an explicit hit test** (`_checkbox_item` / `_on_tree_click`). A ttk `Treeview` has no clickable per-row checkbox; the mark is just a prefix on the item's `#0` text, and without a `<Button-1>` handler clicking it only selected the row. `bbox(item, "#0")` returns the *indented* cell, which still leads with the expand indicator (drawn empty for a childless file row, so those pixels are dead space) — the handler walks right from `bbox[0]` until `identify_element` reports `"text"`, then treats that point plus one mark-width as the hit zone. Measured empirically: indicator ≈18px, mark ≈34px at scale 1.0. Clicks past that zone fall through to normal selection.
- **Maximised state is persisted separately from geometry** (`Settings.maximized`). Once a window is zoomed, `winfo_geometry()` reports the maximised *size* and `wm_geometry()` reports the maximised size with the restored position — neither gives back the restore size, so saving either and reapplying it on launch produced a normal-state window sized like a maximised one. `_on_window_configure` therefore records `_normal_geometry` on every `<Configure>` while the window is not zoomed, and `_apply_settings` re-zooms via `after(0, …)` (zooming an unmapped window discards the restore geometry just set).
- **The scan folder list is deliberately not persisted.** `Settings` has no `folders` field; every launch starts empty so a stale folder can't be swept into a scan. Old settings files still carrying a `"folders"` key load fine — unknown keys are ignored.
...
- **Groups wrap into a contact sheet rather than one row** once an equal share of the panel would fall below `THUMB_MIN` (`_video_box` returns `columns` alongside the box size). This is copied from `../ImageGrouper` and `../DuplicateImageFinder`: the horizontal strip it replaced could only be browsed by scrolling sideways, which the wheel does not do by default; wrapping means a big group is read by scrolling *down*, and measured against the real widget the content now fits horizontally (`xview` is `(0.00, 1.00)`) at every group size tried.
  **The wrapped block is not square here, unlike the sibling image projects** (`THUMB_WRAP_W` 264 × `THUMB_WRAP_H` 110, not one `THUMB_WRAP`). A video's block is itself a 4×2 grid of mini-frames (`_grid_shape`), whose natural aspect for 16:9 footage is ~3.6:1, so a square block would leave the frames tiny with empty bands above and below. 264 gives each mini-frame ~64px — against the ~38px the narrowest *unwrapped* block managed — and still fits three videos across a 900px panel. The height is deliberately taller than 16:9 needs so portrait video is small rather than unreadable.
- **`Gallery._fit_window` must call `update_idletasks()` before measuring**, under the `_fitting` re-entrancy guard. It runs straight after `_render()` grids the cells, before Tk has recomputed the frame's requested size, so it measures a stale value — and pinning the canvas window to that stale size stops the frame's *actual* size from ever changing, so the `<Configure>` binding meant to correct it never fires again and everything past the first screenful is unreachable at any scroll position. Same shipped bug `../ImageGrouper` documents (its `probe/scrollbug.py` reproduces it); it is why scrolling here appeared dead rather than merely awkward, and the wrap alone would not have fixed it.
- `gallery.py`'s caption carries the file's **containing folder** under its name (`folder_caption`/`elide_middle`), same as the image project. A group routinely spans several scanned folders and two copies of one video usually share a file name, so the folder is the only thing in the caption that tells them apart. The character budget scales with cell width so the line stays around two lines; `CAPTION_SPACE` was raised to reserve the room.
